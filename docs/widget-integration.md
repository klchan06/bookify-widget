# Boekgerust Widget - Integratie Handleiding

De Boekgerust widget kan op verschillende manieren worden ingebed op je website.
...
<script src="https://cdn.boekgerust.nl/widget.js" defer></script>

<!-- Stap 2: Plaats het Web Component waar je de widget wilt tonen -->
<boekgerust-widget
  salon-id="jouw-salon-id"
  api-url="https://api.boekgerust.nl"
></boekgerust-widget>
```
...
```html
<iframe
  src="https://widget.boekgerust.nl/embed?salonId=jouw-salon-id"
  width="100%"
  height="600"
  frameborder="0"
...
| `api-url` | string | Ja | - | URL van de Boekgerust API |
...
```php
function boekgerust_widget_script() {
    wp_enqueue_script(
        'boekgerust-widget',
        'https://cdn.boekgerust.nl/widget.js',
        array(),
        null,
        true
    );
}
add_action('wp_enqueue_scripts', 'boekgerust_widget_script');
```

Gebruik vervolgens de shortcode of HTML blok met alleen het `<boekgerust-widget>` element.
...
declare global {
  namespace JSX {
    interface IntrinsicElements {
      'boekgerust-widget': React.DetailedHTMLProps<
...
function BookingPage() {
  ...
  useEffect(() => {
    ...
    script.src = 'https://cdn.boekgerust.nl/widget.js';
    script.defer = true;
...
Ja, gebruik gewoon meerdere `<boekgerust-widget>` elementen met hetzelfde of verschillende salon IDs.
...
<boekgerust-widget id="booking" style="display:none" ...></boekgerust-widget>
```
